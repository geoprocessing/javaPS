...
/**
 * TODO modify this validation process.
 * @author mingda zhang
 */
public class PerfomStepParameterValidator implements ParameterValidator<PerformStepRequest> {
    ...
    @Inject
    public PerfomStepParameterValidator(Engine engine) {
        this.engine = engine;
    }

    @Override
    public void validate(PerformStepRequest request) throws OwsExceptionReport {
    ...
    private void validate(PerformStepRequest request,
    ...
    private void validateOutputs(Collection<OutputDefinition> outputs,
        ...
        outputs.stream().map(OutputDefinition::getId).collect(MoreCollectors.toDuplicateStream()).map(
                PerfomStepParameterValidator::duplicateOutput).forEach(exception::add);
