 ...
 * @author Mingda Zhang
 */
public class PerformStepHandler extends AbstractEngineHandler implements GenericOperationHandler<PerformStepRequest,
        ExecuteResponse> {
    private static final String IDENTIFIER = "Identifier";

    private static final OperationHandlerKey KEY = new OperationHandlerKey(WPSConstants.SERVICE,
            WPSConstants.Operations.PerformStep);

    @Inject
    public PerformStepHandler(Engine engine) {
        super(engine);
    }

    @Override
    public ExecuteResponse handle(PerformStepRequest request) throws OwsExceptionReport {
        ...
        Result result = null;
        try {
            result = getEngine().performStep(jobId, request.getInputs(), request.getOutputs(), request.getResponseMode());
        ...
        if (request.getExecutionMode() == ExecutionMode.SYNC) {
            try {
                Result result1 = getEngine().getResult(jobId).get();
                ExecuteResponse response = new ExecuteResponse(service, version, result);
